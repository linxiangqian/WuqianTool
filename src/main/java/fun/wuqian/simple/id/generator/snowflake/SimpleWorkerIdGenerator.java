...
/**
 * workerid生成器
 * 参考： https://www.jianshu.com/p/7f0661ddd6dd
 * @author linxiangxian
 *
 */
public class SimpleWorkerIdGenerator {
	/**
	 ...
	 *
	 * @author linxiangxian
	 * @return  createWorkerIdByHostName
	 **/
	 public static Long createWorkerIdByHostName() {
	    InetAddress address;
	    Long workerId;
	    try {
	  ...
	  * @return  createWorkerIdByIP
	  */
	 public static long createWorkerIdByIP() {
	     InetAddress address;
	     try {
	 ...
	 /**
	  * 浏览IPKeyGenerator workerId生成的规则后，感觉对服务器IP后10位（特别是IPV6）数值比较约束.
	  * 
	  * <p>
	  * 有以下优化思路：
	  * 因为workerId最大限制是2^10，我们生成的workerId只要满足小于最大workerId即可。
	  * 1.针对IPV4:
	  * ....IP最大 255.255.255.255。而（255+255+255+255) 小于 1024。
	  * ....因此采用IP段数值相加即可生成唯一的workerId，不受IP位限制。
	  * 2.针对IPV6:
	  * ....IP最大ffff:ffff:ffff:ffff:ffff:ffff:ffff:ffff
	  * ....为了保证相加生成出的workerId 小于 1024,思路是将每个bit位的后6位相加。这样在一定程度上也可以满足workerId不重复的问题。
	  * </p>
	  ...
	  * @return  id
	  */
	 public static long createWorkerIdByIPSection() {
	     InetAddress address;
	     try {
	         address = InetAddress.getLocalHost();
	     } catch (final UnknownHostException e) {
